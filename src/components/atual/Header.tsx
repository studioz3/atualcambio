...
export function Header() {
  ...
  return (
    <header
      className={cn(
        "bg-navy-deep fixed inset-x-0 top-0 z-50 text-white transition-all duration-200",
              ...
              {nav.map((item) =>
                "items" in item && item.items ? (
                  ...
                  >
                    ...
                    {openMenu === item.label ? (
                      <div className="absolute top-full left-0 w-64 rounded-md border border-white/10 bg-navy-deep p-2 shadow-[0_12px_40px_rgba(1,24,58,0.45)]">
                        {item.items.map((sub) => (
                          <Link
                            key={sub.label}
                            to={sub.to}
